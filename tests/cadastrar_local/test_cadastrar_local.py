...
def test_cadastrar_local(
    ...
    for index, dados in enumerate(dados_teste):
        ...
        try:
            add_photo.send_keys(f"{local_image_path.absolute()}")
        except:
            driver.execute_script(f'arguments[0].value = "{local_image_path.absolute()}"', local_image_path)
        ...
        assert "/medidas/cadastrar" in driver.current_url
